...
KANE_QuNeo.jumpLoopTimer = 55; // time(ms) to wait before executing a jumped loop 
...
KANE_QuNeo.autoLoopStart = KANE_QuNeo.makeVar(0); // start loop size for autoloop
...
KANE_QuNeo.initAutoLoop = function (deck, factor) {
  ...
  KANE_QuNeo.startAutoLoop = function (deck) {
    ...
    print("starting autoloop")
    var channel = deck - 1;
    var channelName = KANE_QuNeo.getChannelName(deck);
    var numBeats = KANE_QuNeo.autoLoopStart[channel];
    ...
    var timer = (spb * 1000) // need milliseconds per beat
  ...
  case 4:
  KANE_QuNeo.loopHoldTimers[channel].push(
   engine.beginTimer(timer, "KANE_QuNeo.doAutoLoop4("+deck+")")); break;
  case 8:
  KANE_QuNeo.loopHoldTimers[channel].push(
  ...
  KANE_QuNeo.stopAutoLoop = function (deck) {
   ...
   KANE_QuNeo.deckMultiplyLoop = function (deck, factor, status) {
    ...
    engine.beginTimer(KANE_QuNeo.autoLoopHoldTime,
      "KANE_QuNeo.initAutoLoop("+deck+","+factor+")",
      true));
    ...
    var channelName = KANE_QuNeo.getChannelName(deck);
       ...
       if (engine.getValue(channelName, "beatloop_"+x+"_enabled")) {
	     ...
	     KANE_QuNeo.assertJumpSyncLED(deck); // reset LED
